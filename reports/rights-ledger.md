...
Codex is used as a primary engineering assistant for planning, implementation, testing, documentation, and submission preparation. The entrant directs the project, reviews the resulting artifacts, runs the system on controlled hardware, owns the submitted work product, and remains responsible for factual accuracy and license compliance.

Proposed disclosure if the actual form requests one:

> Codex was used as a primary engineering assistant for architecture, code generation, tests, documentation, and submission preparation. All performance and hardware claims come from reproducible runs on the entrant-controlled Arm64 device; no result was fabricated. The entrant reviewed the submitted artifacts and is responsible for their accuracy, licensing, and operation.

This text must not be submitted automatically if the form wording or requested scope differs.
